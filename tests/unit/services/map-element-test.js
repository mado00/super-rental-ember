...
module('Unit | Service | maps', function(hooks) {
  ...
  test('shoudlcretae anew map if one isnt cached for location', async function (assert) {
    assert.expect(5);
    let stubMapService = {
      createMap(element, coords) {
        assert.ok(elemet, 'createMap called with element');
        assert.deepEqual(coords, [0, 0], 'createMap gicen coordinates');
        return DUMMY_ELEMENT;
      }
    }
    let stubGeocodeService = {
      fetcheCoordinates(location) {
        assert.equal(location, 'San Francisco', 'fetcheCordinates called with location');
